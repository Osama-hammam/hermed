...
export default function AdminLogin() {
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      ...
      else setError("Invalid credentials. Try admin@hermed.com / hermed2024");
      setLoading(false);
    }, 600);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-brand-900 via-brand-800 to-brand-600 flex items-center justify-center px-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 text-sm mt-1">Sign in to manage HERMED</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
              placeholder="admin@hermed.com" className="input" required />
          </div>
          <div>
        ...
        <div className="mt-6 p-4 bg-slate-50 rounded-xl text-xs text-slate-400">
          ...
          <p>Email: <span className="font-mono text-slate-600">admin@hermed.com</span></p>
          <p>Password: <span className="font-mono text-slate-600">hermed2024</span></p>
        </div>
      </div>
    </div>
  );
